...
def cctree_to_link_nodes_and_edges(path):
    with open(path) as fle:
        ...
        if not len(lines):
            print('Empty CCTree file')
            return [], []

        j = lines[0].find('+-')
        if j == -1 or j + 2 >= len(lines[0]):
            print('Bad format of CCTree file')
            return [], []
        ...
        for line in lines:
            off = line.find('+-')
            if off == -1:
                print('Bad format of CCTree file:', line)
                return [], []
            offs.append(off)
        ...
        for i, _ in enumerate(lines):
            ...
            for j in range(i - 1, root-1, -1):
                if offs[j] >= offs[i]:
                    ...
                    print('Bad format of CCTree file:', ''.join(lines[j:i+1]))
                    return [], []
                else:
                    found = True
            ...
            if not found:
                ...
                if lines[i][offs[i] + 2] == '<':
                    ...
                    print('Bad format of CCTree file:', lines[0])
                    return [], []
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("usage: %s <path/to/graph.cct> [<path/to/graph.dot>]\n" % (sys.argv[0]))
        exit(1)
